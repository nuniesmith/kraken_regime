...
use kraken_regime::integration::rest_integration;
...
async fn handle_trade_signal(action: TradeAction) {
    match action.action {
        TradeType::Buy => {
            ...
            info!("   Stop Loss: {:?}", action.stop_loss.map(|p| format!("${:.2}", p)));
            info!("   Take Profit: {:?}", action.take_profit.map(|p| format!("${:.2}", p)));
            info!("   Strategy: {}", action.source_strategy);
            info!("   Regime: {} (confidence: {:.1}%)", action.regime, action.confidence * 100.0);
