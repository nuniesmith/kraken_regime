...
use kraken_regime::regime::{
    EnsembleRegimeDetector, HMMConfig, HMMRegimeDetector, MarketRegime, RegimeDetector,
    TrendDirection,
};
...
fn test_bull_market() {
    ...
    let (indicator_results, hmm_results, ensemble_results) = run_scenario(
        |i, price| {
...
fn run_scenario<F>(
    price_fn: F,
    n_bars: usize,
) -> (Vec<MarketRegime>, Vec<MarketRegime>, Vec<MarketRegime>)
where
    F: Fn(usize, f64) -> f64,
{
    // Create detectors
...
fn print_results(
    ...
    println!(
        "  │ Indicators  │ {:<46} │",
        format_counts(&ind_counts, total)
    );
    println!(
        "  │ HMM         │ {:<46} │",
        format_counts(&hmm_counts, total)
    );
    println!(
        "  │ Ensemble    │ {:<46} │",
        format_counts(&ens_counts, total)
    );
    println!("  └─────────────┴────────────────────────────────────────────────┘");
    ...
    println!();
    println!(
        "  Agreement Rate (Indicator vs HMM): {:.1}%",
        agreement_rate
    );
}
