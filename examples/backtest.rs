...
impl BacktestResults {
    ...
    fn print_summary(&self) {
        ...
        println!(
            ...
            self.strategy_usage.trend_following_signals
        );
        println!(
            "   Mean Reversion:   {} signals",
            self.strategy_usage.mean_reversion_signals
        );
        println!(
            "   No Trade:         {} periods",
            self.strategy_usage.no_trade_periods
        );
...
fn run_backtest(data: &[Candle]) -> BacktestResults {
    ...
    for candle in data {
        ...
        if let Some(action) = trader.process_candle("BTC/USD", candle) {
            results
                .strategy_usage
                .record_signal(&kraken_regime::strategy::router::RoutedSignal {
                    ...
                    source_strategy: match action.source_strategy.as_str() {
                        "Trend Following" => {
                            kraken_regime::strategy::router::ActiveStrategy::TrendFollowing
                        }
                        "Mean Reversion" => {
                            kraken_regime::strategy::router::ActiveStrategy::MeanReversion
                        }
                        _ => kraken_regime::strategy::router::ActiveStrategy::NoTrade,
                    },
                    regime: trader
                        .get_regime("BTC/USD")
                        .unwrap_or(MarketRegime::Uncertain),
            ...
            if let Some(ref pos) = position {
                if let Some(stop) = pos.stop_loss {
                    if candle.low <= stop {
                        pnl = (stop - pos.entry_price) / pos.entry_price * pos.size;
                        hit_stop = true;
                    }
                }
                if let Some(tp) = pos.take_profit {
                    if candle.high >= tp {
                        pnl = (tp - pos.entry_price) / pos.entry_price * pos.size;
                        hit_tp = true;
                    }
                }
            }
