...
use crate::regime::{MarketRegime, RegimeConfig};
...
impl KrakenRegimeTrader {
    ...
    pub fn process_candle(&mut self, pair: &str, candle: &Candle) -> Option<TradeAction> {
        ...
        self.last_candle_time.insert(pair.to_string(), candle.timestamp);
        
        // Update router and get signal
        let routed = self.router.update(pair, candle.high, candle.low, candle.close)?;
        ...
        if action.action != TradeType::Hold {
            if let Some(tx) = &self.signal_tx {
                ...
                tokio::spawn(async move {
                    let _ = tx.send(action_clone).await;
                });
            }
        }
    ...
    pub fn process_tick(&mut self, pair: &str, price: f64, timestamp: i64) -> Option<TradeAction> {
        ...
        let builder = self.candle_builders
    ...
    pub fn warmup_with_history(&mut self, pair: &str, candles: &[Candle]) {
        println!("[{}] Warming up with {} historical candles", pair, candles.len());
    ...
    pub fn status_summary(&self) -> HashMap<String, PairStatus> {
        ...
        for pair in &self.config.pairs {
            status.insert(pair.clone(), PairStatus {
                    ...
                    if let Some(ohlc) = item.as_array() {
                        if ohlc.len() >= 6 {
                            let candle = Candle {
                                ...
                                volume: ohlc[6].as_str().and_then(|s| s.parse().ok()).unwrap_or(0.0),
                            };
                            candles.push(candle);
                        }
                    }
